...
/*
RamdomInitTestPaper实现GiveTestPaper接口，负责给出试卷
随机从题库中获得若干试题
 */
public class RamdomInitTestPaper implements GiveTestPaper{
    ...
    Sheet sheet = null;//封装 Excel中的 sheet，Sheet是jxl包中的类(工作表)
    ...
    private void randomGiveProblem(int amount) {//随机给出amount道试题,放入problem数组中
        ...
        for(int i=0;i<problem.length;i++){
            ...
            int index =list.remove(m);//删除 list的第 m个节点，同时得节点数字

            //Cell:代表工作表中的单个单元格。可以查询其类型和内容
            Cell[] cell = sheet.getRow(index);//返回 sheet中的第 index行
